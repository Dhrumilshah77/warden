...
await check("static app shell", async () => {
  ...
  assert(appJs.includes("Owner Analytics"), "owner analytics should be rendered in the UI");
...
await check("delegated action policy changes by user", async () => {
  ...
  assert(autonomy.customers.some((customer) => customer.tags.includes("regular")), "autonomy plan should include regular customers");
